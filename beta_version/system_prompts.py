...
"""
# i like the script system prompt for the most part but i think that it needs some changing.
# Instead of generating the image descriptions for the entire video, i want a version of
# this that only generates descriptions for a single segment.
# input should be {script_segment: , full_script, additional_image_requests, image_style, topic, tone, num_of_clips}
# output should be {image_descriptions: [{description: , uses_logo: }] ( the same as it is now). main difference is that it is doing for segments now and not the whole script.
# it can take inspiration from this system prompt that i have for a different purpose that generates images for specicifc segment
# Anything you think might be helpful to take from this is good. i still want for the descriptions, even tho its for a segment, to be as detailed as possible. it shouldnt have a specicifc word limit of how detailed it should be.
# Also included in each of the descriptions should be a reference to the image style etc as he images will be fed to an image genrator separately. The system prompt that i said it may take inspiration from is as follows:
base_image_descriptions_generator_system_prompt_no_face = """
...

{
  ...
  "clip_voice_script": "<string — voice over text for the current segment>",
...
7) **CRITICAL: Ensure NO faces are visible in any description. For all humans and animals, specify that faces are covered, obscured, turned away, or out of frame.**
...
- **FACE AVOIDANCE (PRIMARY CONSTRAINT): When depicting humans or animals, always specify one of the following:**
  - Face turned away or facing backward
  - Face completely covered (hat, helmet, mask, hood, hair covering)
  - Face obscured by object or action (hands, arms, tools, debris, foliage)
  - Shot from behind or side angle where face is not visible
  - Silhouette composition
  - Only partial body visible (hands, torso, feet, legs) with face out of frame
  - Long shot or overhead angle where facial features are indistinct
  - Describe these face-avoidance techniques explicitly in the description to ensure compliance.



DESCRIPTION CHECKLIST (cover these explicitly and concisely in each string)

1) SUBJECT & ACTION  

   - Main focus (objects, scenes, products, settings), their actions and context.
   - If humans or animals are present, explicitly state how their faces are NOT visible (covered, turned away, obscured, out of frame, etc.).
   - Supporting elements that reinforce the VO beat.  
...
3) COMPOSITION  
   ...
   - Focal hierarchy, rule of thirds, leading lines, text-safe areas.
   - Angle choices that naturally avoid faces (over-shoulder, behind, below-angle, etc.).  
...
8) NEGATIVE/AVOID LIST  
   ...
   - "Avoid cluttered backgrounds, misshapen anatomy, unreadable text, watermarks, copyrighted IP, and visible faces or facial features."
...
- Human/animal subjects: Prioritize composition angles, positioning, and framing techniques that keep faces completely out of view.
...
- **Does every description with humans or animals explicitly specify how faces are avoided/obscured?**
